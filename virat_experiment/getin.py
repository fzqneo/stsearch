...
def query(path, session):
    ...
    def merge_op_getin(ic, ip):
        new_bounds = ic.bounds.span(ip)
        new_bounds['t1'] = max(0, ip['t2'] - 1*fps) 
        new_bounds['t2'] = min(frame_count, ip['t2'] + 3*fps)
        new_payload = {rekey: ip.payload[rekey]}
        return Interval(new_bounds, new_payload)

    get_out = JoinWithTimeWindow(
        lambda ic, ip: ic['t1'] < ip['t2'] < ic['t2'] and _iou(ic, ip.payload[rekey][-1]) > 0.05,
        merge_op=merge_op_getin,
        window=5*60*fps
...
def main(mode, path=None, result_file="getin_result.csv", get_mp4=True, mp4_dir="getin_mp4"):
