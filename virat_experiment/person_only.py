...
def query(path, session):
    ...
    frames_with_person = DetectionFilter(['person'], 0.3)(detections)

    merged_volume = Coalesce(epsilon=3*detect_step)(frames_with_person)
    merged_volume = Filter(lambda i: i.bounds.length()>1*fps)(merged_volume)

    output = merged_volume # Don't material fg here to avoid OOM
...
if __name__ == "__main__":
    ...
    for i, res in enumerate(results):
        ...
        for b, mp4 in query_result['results']:
            ...
            open(f"person_only_{clip_id}_{b['t1']}_{b['t2']}.mp4", 'wb').write(mp4)
