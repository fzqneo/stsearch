...
def query(path, session):
    ...
    two_person = Join(
        predicate=lambda i1, i2: i1['t1'] == i2['t1'] and i1['x1'] < i2['x1'],
        merge_op=merge_op,
        window=detect_step
    )(person_1, person_2)

    # dedup and merge final results
    two_person = Coalesce(epsilon=detect_step)(two_person)

    if GET_MP4:
        vis_decoder = LRULocalVideoDecoder(path, cache_size=300)
        raw_fg = VideoCropFrameGroup(vis_decoder, copy_payload=True)(two_person)
        output = raw_fg
    else:
        output = two_person
...
def main(mode, path=None, result_file="two_person_result.csv", get_mp4=True, mp4_dir="two_person_mp4"):
